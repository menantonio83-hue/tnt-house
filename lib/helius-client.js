...
export async function performFullAudit(mintAddress) {
  try {
    ...
    // checkHolderDistributionRisk returns riskLevel 'ERROR' with zeroed
    // percentages when it could not read holders. Those zeros do not trigger
    // the concentration caps in lib/scoring.ts — they disable them, because
    ...
    const capResult = applyScoreCaps(
      ...
      null, // rugged: not checked in Quick Check
      {
        ...
        // v1.3 of lib/scoring.ts: Quick Check knows the authorities from
        // the mint account, so the new authority caps apply here too;
        // honeypot/LP data is NOT checked in Quick Check, so those stay
        // null (cap cannot fire) — same honesty rule as before.
        honeypotRisk: null,
