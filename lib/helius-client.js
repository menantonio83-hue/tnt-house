// Version 1.3 — lib/helius-client.js
...
//
// v1.3 (2026-09-12): Quick Check no longer builds its own holder picture
// from a bare getTokenLargestAccounts on the public RPC. That path could
// silently turn an RPC failure into a fake worst-case reading
// (riskLevel 'CRITICAL' with 100/100 percentages) on large, perfectly
// healthy tokens like BONK. Quick Check now uses exactly the data
// sources the paid Risk-Data API uses:
//
//   * holders -> lib/holder-distribution.ts getHolderDistributionRobust
//     (retries, Helius RPC, honest 'ERROR' on failure)
//   * honeypot / LP lock / LP burn / taxes / dev wallet / hidden owner /
//     permanent delegate / Jupiter verification -> lib/rugcheck-client.ts
//     getRugCheckRiskData (one RugCheck /report call, same as the API)
//   * mint/freeze authority -> same direct RPC getMintInfoResult as before
//
// Still NOT done in Quick Check (deliberately, for speed): insider-
// cluster tracing (clusterAnalysis stays 'pending' — neutral +12 in the
// base, never a fabricated verdict) and Streamflow vesting-lock
// detection (Risk-Data API only). Both are documented in the UI.
//
// The old checkHolderDistributionRisk/getTopHolders functions remain
// exported below for backward compatibility but are no longer wired into
// performFullAudit — do not wire them back; use getHolderDistributionRobust.

import {
  isHolderReadingUnusable,
  HOLDER_DATA_UNAVAILABLE_ERROR,
  HOLDER_DATA_UNAVAILABLE_MESSAGE,
} from './holder-data-guard';
...
// v1.3 — Quick Check now shares the Risk-Data API's data sources.
import { getHolderDistributionRobust } from './holder-distribution';
import { getRugCheckRiskData } from './rugcheck-client';
import { withTimeout } from './with-timeout';

// v1.3: prefer Helius RPC (the endpoint the rest of the system already
// uses) and fall back to the public node exactly as before.
...
// v1.3 — Quick Check budgets. getHolderDistributionRobust has its own
// internal retries and per-call timeouts; this outer cap keeps the whole
// "quick" check quick. On timeout it resolves to the ERROR shape below,
// which the guard refuses to score (and the route refunds the slot) — an
// honest "couldn't read" instead of a fabricated number.
const QC_HOLDER_TIMEOUT_MS = 10000;
const QC_HOLDER_FALLBACK = {
  riskLevel: 'ERROR',
...
export async function performFullAudit(mintAddress) {
  try {
    ...
    // v1.3: the same four data sources the Risk-Data API fetches, minus
    // the cluster trace. getRugCheckRiskData is one HTTP call to RugCheck
    // that covers honeypot, LP lock/burn, taxes, dev wallet, hidden
    // owner, permanent delegate and Jupiter verification — none of which
    // the old Quick Check could see at all.
    const [mintResult, holderRisk, dexData, rugCheckData] = await Promise.all([
      getMintInfoResult(mintAddress),
      withTimeout(getHolderDistributionRobust(mintAddress), QC_HOLDER_TIMEOUT_MS, QC_HOLDER_FALLBACK),
      getDexScreenerData(mintAddress),
      getRugCheckRiskData(mintAddress),
    ]);
    ...
    // Same rule as the !mintResult branch below, for the holder read.
    // getHolderDistributionRobust (and the QC_HOLDER_TIMEOUT_MS outer cap
    // above) report riskLevel 'ERROR' with zeroed percentages when the
    // holders could not be read. Those zeros do not trigger the
    // concentration caps in lib/scoring.ts — they disable them, because
    // zero is below every threshold. Treated as an rpc_failure so Quick Check
    // refunds the slot: the user did nothing wrong.
    if (isHolderReadingUnusable(holderRisk)) {
      console.error(`[helius-client] ${mintAddress}: holder reading unusable, refusing to score`);
      return {
        mintAddress,
        auditAvailable: false,
        failureReason: 'rpc_failure',
        securityScore: null,
        verdict: '⚠️ AUDIT UNAVAILABLE',
        verdictColor: 'gray',
        error: HOLDER_DATA_UNAVAILABLE_ERROR,
        message: HOLDER_DATA_UNAVAILABLE_MESSAGE,
    ...
    //
    // v1.3: the cap inputs now come from the same places the Risk-Data
    // API gets them — RugCheck for contract/market red flags (real
    // honeypot/LP/tax/dev-wallet/Jupiter data instead of the old all-
    // nulls) and getHolderDistributionRobust for concentration. The only
    // input still left as unknown is the insider-cluster trace, passed as
    // 'pending' (neutral +12 in the base) rather than a false-clean
    // default — same honesty rule as always: never assert what we did
    // not check. RugCheck-null fields stay null in the caps, and a null
    // input simply cannot fire its cap.
    ...
    const capResult = applyScoreCaps(
      ...
      rugCheckData.rugged,
      {
        ...
        tokenProgram: rugCheckData.token_program ?? (extensions.standardProgram ? 'standard' : 'nonstandard'),
        buyTaxPercent: rugCheckData.buy_tax_percent ?? extensions.buyTaxPercent,
        sellTaxPercent: rugCheckData.sell_tax_percent ?? extensions.sellTaxPercent,
        devWalletPercent: rugCheckData.dev_wallet_percent,
        honeypotRisk: rugCheckData.honeypot_risk,
        mintAuthorityActive: !mintAuthRevoked,
        freezeAuthorityActive: !freezeAuthRevoked,
    ...
    return {
      ...
      checks: {
        ...
        // v1.3 — real RugCheck signals, previously all Unknown in Quick Check.
        honeypotRisk: rugCheckData.honeypot_risk,
        lpLock: rugCheckData.lp_locked,
        lpBurned: rugCheckData.lp_burned,
        jupiterVerified: rugCheckData.jup_verified,
        rugged: rugCheckData.rugged,
        devWalletPercent: rugCheckData.dev_wallet_percent,
        hiddenOwner: rugCheckData.hidden_owner,
        permanentDelegate: rugCheckData.permanent_delegate,
        // Always 'pending' — Quick Check never runs the cluster trace.
      ...
      // v1.3 — the same cap diagnostics the Risk-Data API exposes, so a
      // Quick Check result can be explained exactly like an API response.
      capsTriggered: capResult.capsTriggered,
      dominantCap: capResult.dominantCap,
      reliefsTriggered: capResult.reliefsTriggered,
      ...
      dbFields: {
        ...
        lp_locked_percent: rugCheckData.lp_locked
          ? Number(rugCheckData.lp_locked.percent.toFixed(2))
          : null,
        // v1.3: vesting-lock detection is not part of Quick Check (see the
        // v1.3 header) — left honest as null rather than implying a
        // confirmed zero.
        vesting_locked_percent: null,
        buy_tax_percent: rugCheckData.buy_tax_percent ?? extensions.buyTaxPercent,
        sell_tax_percent: rugCheckData.sell_tax_percent ?? extensions.sellTaxPercent,
        contract_renounced: mintAuthRevoked && freezeAuthRevoked,
        hidden_owner:
          rugCheckData.hidden_owner === true
            ? 'Yes 🚨'
            : rugCheckData.hidden_owner === false
              ? 'No ✓'
              : 'Unknown',
        age_days: dexData.ageDays,
        creator_balance_percent: rugCheckData.dev_wallet_percent,
