...
export default function TntHouse() {
  ...
  // Safe MRDT price getter — never returns 0 or NaN, fallback to $0.000013
  var getSafeMrdtPrice = function() {
    var p = mrdtPriceRef.current || mrdtPrice || 0.000013;
    return (p > 0 && isFinite(p) && !isNaN(p)) ? p : 0.000013;
  };

  // Safe SOL price getter — fallback to $150
  var getSafeSolPrice = function() {
    var p = solPriceRef.current || solPrice || 150;
    return (p > 0 && isFinite(p) && !isNaN(p)) ? p : 150;
  };

  // Returns amount in MRDT or SOL depending on selected payment method
  var getAmountForTier = function(tier, method) {
    var usd = tier === 'fast' ? 25 : tier === 'vip' ? 75 : 10;
    if (method === 'SOL') {
      var sp = getSafeSolPrice();
      return parseFloat((usd / sp).toFixed(4));
    }
    var mp = getSafeMrdtPrice();
    return parseFloat((usd / mp).toFixed(2));
  };

  // Returns amount in MRDT or SOL depending on selected payment method
  var getAmountForBanner = function(days, method) {
  ...
  var isMobile = function() { return /Android|iPhone|iPad|iPod/i.test(navigator.userAgent); };
  ...
  // Build Solana Pay Transaction Request URI pointing to our API
  // Phantom/Solflare will GET metadata then POST with user pubkey → we return serialized tx
  var buildTransactionRequestUri = function(amount, method, label) {
    var apiUrl = 'https://tnt-audit.com/api/pay?amount=' + amount + '&method=' + method + '&label=' + encodeURIComponent(label);
    return 'solana:' + encodeURIComponent(apiUrl);
  };

  // Trigger payment — Transaction Request API (works on all platforms)
  // Phantom fetches tx from our server → shows exact amount → user signs
  var triggerPayment = function(amount, method, label, message, wallet) {
    var txRequestUri = buildTransactionRequestUri(amount, method, label);

    // 1. Inside Phantom/Solflare in-app browser — Transaction Request works directly
    if (isInsidePhantom() || isInsideSolflare()) {
      window.location.href = txRequestUri;
      return;
    }

    // 2. Mobile browser — open via Phantom Universal Link (iOS) or browse intent (Android)
    if (isMobile()) {
      var isSolflare = wallet === 'Solflare';
      if (isSolflare) {
        // Solflare Universal Link with Transaction Request
        window.location.href = 'https://solflare.com/ul/v1/browse?url=' + encodeURIComponent(txRequestUri);
      } else {
        // Phantom Universal Link with Transaction Request
        // Opens Phantom in-app browser which processes the Transaction Request
        window.location.href = 'https://phantom.app/ul/browse?url=' + encodeURIComponent('https://tnt-audit.com/api/pay?amount=' + amount + '&method=' + method + '&label=' + encodeURIComponent(label));
      }
      return;
    }

    // 3. Desktop — raw Transaction Request URI
    window.open(txRequestUri, '_blank');
  };
  ...
  // Fetch MRDT price from DexScreener + SOL price from CoinGecko
  useEffect(function() {
    var fetchPrices = async function() {
      // MRDT price
      try {
      ...
      // SOL price from CoinGecko (free endpoint)
      try {
        var solRes = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=solana&vs_currencies=usd');
        var solData = await solRes.json();
        if (solData && solData.solana && solData.solana.usd) {
          var sp = parseFloat(solData.solana.usd);
          if (sp > 0 && isFinite(sp) && !isNaN(sp)) {
            setSolPrice(sp);
    ...
    fetchPrices();
    var i = setInterval(fetchPrices, 60000);
  ...
  var handlePaymentMethodSelect = function(method) {
    ...
    // Calculate amount based on chosen method
    var amount = getAmountForTier(selectedTier, method);
  ...
  var handleBannerSubmit = function(e) {
    ...
    setBannerInvoiceUsd(usd); setBannerError('');
    setShowBannerPaymentModal(true);
  };

  var handleBannerPaymentMethodSelect = function(method) {
    setSelectedBannerPaymentMethod(method);
    var amount = getAmountForBanner(bannerFormData.days, method);
    setBannerInvoiceAmount(amount);
  ...
  // Display label for invoice — shows token symbol and USD-equivalent amount
  var getInvoiceCurrencyLabel = function(method) {
    return method === 'SOL' ? 'SOL' : '$MRDT';
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white font-mono relative overflow-hidden pb-12">
      ...
      <div className="relative z-10">
          ...
          <div className="max-w-7xl mx-auto px-6 py-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-[10px] text-purple-400 block font-bold tracking-widest">TOP NEW TOKENS v0.4</span>
              </div>
            </div>
        ...
        {/* BANNER SECTION — clean, no ad text */}
          ...
          {activeBanner ? (
            <>
              <div className="relative border border-purple-500/40 rounded-2xl overflow-hidden shadow-[0_0_30px_rgba(168,85,247,0.25)] min-h-[160px]">
                ...
                {/* Clean banner — only token name, slogan, countdown. No "VIP BOOST" badge. */}
                <div className="relative z-10 p-6 flex flex-col justify-end min-h-[160px]">
                  <h4 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-300 to-emerald-300">${activeBanner.tokenName}</h4>
                  <p className="text-slate-300 text-sm mt-1">{activeBanner.desc}</p>
                  {bannerCountdown && (
                    <p className="text-[10px] text-slate-500 mt-2">⏱ slot available in <span className="text-purple-400 font-bold">{bannerCountdown}</span></p>
                  )}
                </div>
              </div>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-start">
                ...
                <form onSubmit={handleFormSubmit} className="space-y-4">
                  ...
                  <div><label className="block text-purple-400 text-xs font-bold mb-1">{t.fieldTier}</label>
                    <select value={selectedTier} onChange={function(e) { setSelectedTier(e.target.value); }} className="w-full bg-slate-950 border border-purple-500/20 rounded px-3 py-2 text-xs text-white focus:border-purple-500 focus:outline-none font-mono">
                      <option value="basic">{t.tierBasic} — $10</option>
                      <option value="fast">{t.tierFast} — $25</option>
                      <option value="vip">{t.tierVIP} — $75</option>
                    </select>
                  </div>
                  ...
                  {submitted && <div className="p-3 bg-emerald-950/40 border border-emerald-500/30 rounded text-emerald-300 text-xs text-center font-bold">Audit complete! Token added to table.</div>}
                </form>
              </div>
                ...
                <form onSubmit={handleBannerSubmit} className="space-y-4">
                  ...
                  <div><label className="block text-purple-400 text-[11px] font-bold mb-1">{t.fieldDuration}</label>
                    <select value={bannerFormData.days} onChange={function(e) { setBannerFormData(Object.assign({}, bannerFormData, { days: e.target.value })); }} className="w-full bg-slate-950 border border-purple-500/20 rounded px-3 py-2 text-xs text-white focus:border-purple-500 focus:outline-none font-mono">
                      <option value="1">{t.dur1} — $20</option>
                      <option value="2">{t.dur2} — $35</option>
                      <option value="6">{t.dur6} — $100</option>
                    </select>
                  </div>
            ...
            <div className="space-y-4 bg-slate-900/20 border-2 border-purple-500/20 rounded-xl p-6">
              ...
              <div className="mt-6 space-y-3">
                ...
                <div className="mt-4 p-3 bg-purple-950/30 border border-purple-500/20 rounded-lg text-[10px] text-purple-300 text-center">
                  💡 Pay in $MRDT or SOL — amount calculated at current price
                </div>
              </div>
            </div>
          </div>
        ...
        <footer className="border-t border-purple-500/20 mt-12 py-8 bg-slate-950/60 backdrop-blur-lg">
          <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="text-center space-y-1">
              <div className="text-purple-400 font-bold text-sm tracking-widest">TNT HOUSE v0.4</div>
          ...
          <div className="bg-slate-950 border-2 border-purple-500/40 rounded-2xl w-full max-w-md p-6 shadow-[0_0_40px_rgba(168,85,247,0.25)]" onClick={function(e) { e.stopPropagation(); }}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <button onClick={function() { handlePaymentMethodSelect('MRDT'); }} className="bg-purple-500/10 border-2 border-purple-500/30 hover:border-purple-500 rounded-xl p-6 text-center transition group">
                ...
                <div className="text-[10px] text-purple-300 mt-1 font-mono">~{priceLoading ? '...' : getAmountForTier(selectedTier, 'MRDT').toLocaleString()} MRDT</div>
              </button>
              <button onClick={function() { handlePaymentMethodSelect('SOL'); }} className="bg-emerald-500/10 border-2 border-emerald-500/30 hover:border-emerald-500 rounded-xl p-6 text-center transition group">
                ...
                <div className="text-[10px] text-emerald-300 mt-1 font-mono">~{priceLoading ? '...' : getAmountForTier(selectedTier, 'SOL')} SOL</div>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[100] p-4" onClick={function() { setShowInvoiceModal(false); }}>
          <div className="bg-slate-950 border-2 border-purple-500/40 rounded-2xl w-full max-w-md p-6 shadow-[0_0_40px_rgba(168,85,247,0.25)]" onClick={function(e) { e.stopPropagation(); }}>
            ...
            <div className="bg-slate-900 border border-purple-500/20 rounded-xl p-6 text-center space-y-4">
              ...
              <div className="text-3xl font-black text-emerald-400">{invoiceAmount.toLocaleString()} {getInvoiceCurrencyLabel(selectedPaymentMethod)}</div>
              <div className="text-sm font-bold text-slate-300">≈ ${invoiceUsd} USD</div>
              <div className="text-xs text-slate-500 font-mono break-all">To: {WALLET_ADDRESS.slice(0,8)}...{WALLET_ADDRESS.slice(-8)}</div>
            </div>
            <div className="mt-2 p-2 bg-purple-950/30 border border-purple-500/20 rounded-lg text-[10px] text-purple-300 text-center">Tapping Pay Now will open {selectedWallet}. Confirm the transaction and return here.</div>
          ...
          <div className="bg-slate-950 border-2 border-emerald-500/40 rounded-2xl w-full max-w-md p-6 shadow-[0_0_40px_rgba(16,185,129,0.2)]" onClick={function(e) { e.stopPropagation(); }}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <button onClick={function() { handleBannerPaymentMethodSelect('MRDT'); }} className="bg-purple-500/10 border-2 border-purple-500/30 hover:border-purple-500 rounded-xl p-6 text-center transition group">
                ...
                <div className="font-bold text-purple-400 group-hover:text-white transition">$MRDT</div>
                <div className="text-[10px] text-slate-500 mt-1">{t.recommended}</div>
                <div className="text-[10px] text-purple-300 mt-1 font-mono">~{priceLoading ? '...' : getAmountForBanner(bannerFormData.days, 'MRDT').toLocaleString()} MRDT</div>
              </button>
              <button onClick={function() { handleBannerPaymentMethodSelect('SOL'); }} className="bg-emerald-500/10 border-2 border-emerald-500/30 hover:border-emerald-500 rounded-xl p-6 text-center transition group">
                ...
                <div className="font-bold text-emerald-400 group-hover:text-white transition">SOL</div>
                <div className="text-[10px] text-slate-500 mt-1">Solana</div>
                <div className="text-[10px] text-emerald-300 mt-1 font-mono">~{priceLoading ? '...' : getAmountForBanner(bannerFormData.days, 'SOL')} SOL</div>
              </button>
            </div>
          </div>
        </div>
      ...
      {showBannerInvoiceModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[100] p-4" onClick={function() { setShowBannerInvoiceModal(false); }}>
          <div className="bg-slate-950 border-2 border-emerald-500/40 rounded-2xl w-full max-w-md p-6 shadow-[0_0_40px_rgba(16,185,129,0.2)]" onClick={function(e) { e.stopPropagation(); }}>
            ...
            <div className="bg-slate-900 border border-emerald-500/20 rounded-xl p-6 text-center space-y-4">
              <div className="text-xs text-emerald-400 font-bold">{selectedBannerWallet} · {selectedBannerPaymentMethod}</div>
              <div className="text-3xl font-black text-emerald-400">{bannerInvoiceAmount.toLocaleString()} {getInvoiceCurrencyLabel(selectedBannerPaymentMethod)}</div>
      ...
      {showVerifyModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[200] p-4">
          <div className="bg-slate-950 border-2 border-purple-500/40 rounded-2xl w-full max-w-md p-8 shadow-[0_0_40px_rgba(168,85,247,0.25)] text-center">
            {verifyStatus === 'waiting' && (
              <>
                <RefreshCw className="w-12 h-12 text-purple-400 animate-spin mx-auto mb-4" />
                <h3 className="text-xl font-black text-purple-400 mb-2">{t.waitPayment}</h3>
                <p className="text-slate-400 text-sm mb-4">{t.waitSub}</p>
                <div className="text-xs text-slate-500">{t.checkingBC} {verifyAttempts}/30</div>
                <div className="text-xs text-slate-600 mt-1">{t.timeoutIn} {Math.max(0, Math.ceil((300 - verifyAttempts * 10) / 60))} {t.min}</div>
              </>
            )}
            {verifyStatus === 'success' && (
              <>
                <CheckCircle className="w-12 h-12 text-emerald-400 mx-auto mb-4" />
                <h3 className="text-xl font-black text-emerald-400 mb-2">{t.payConfirmed}</h3>
                <p className="text-slate-300 text-sm">{verifyType === 'banner' ? t.bannerLiveMsg : t.tokenAdded}</p>
              </>
            )}
            {verifyStatus === 'failed' && (
              <>
                <XCircle className="w-12 h-12 text-red-400 mx-auto mb-4" />
                <h3 className="text-xl font-black text-red-400 mb-2">{t.payNotDetected}</h3>
                <p className="text-slate-400 text-sm mb-4">{t.payNotMsg}</p>
                <div className="flex gap-3">
                  <a href="https://t.me/tnt_house2026" target="_blank" rel="noopener noreferrer" className="flex-1 px-5 py-2.5 text-sm rounded-lg bg-purple-500/20 border border-purple-500/40 hover:bg-purple-500/30 transition text-purple-300 font-bold">{t.contactAdmin}</a>
                  <button onClick={function() { setShowVerifyModal(false); }} className="flex-1 px-5 py-2.5 text-sm rounded-lg border border-slate-600 hover:bg-slate-800 transition text-slate-300">{t.close}</button>
                </div>
              </>
            )}
          </div>
      ...
      <div className="fixed bottom-6 right-6 z-[300]">
        {isChatOpen ? (
          <div className="w-80 h-[440px] bg-slate-950 border-2 border-purple-500/40 rounded-2xl shadow-[0_0_30px_rgba(168,85,247,0.25)] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-3 border-b border-purple-500/20 bg-purple-500/10">
              <div className="flex items-center gap-2"><Sparkles className="w-4 h-4 text-purple-400" /><span className="text-xs font-black text-purple-400">TNT AI Inspector</span></div>
              <button onClick={function() { setIsChatOpen(false); }} className="text-slate-400 hover:text-white"><X className="w-4 h-4" /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
              {chatMessages.map(function(msg, i) {
                return (
                  <div key={i} className={'flex ' + (msg.sender === 'user' ? 'justify-end' : 'justify-start')}>
                    <div className={'max-w-[85%] px-3 py-2 rounded-xl text-[11px] whitespace-pre-wrap ' + (msg.sender === 'user' ? 'bg-purple-500/30 text-purple-100' : 'bg-slate-800 text-slate-200')}>{msg.text}</div>
                  </div>
                );
              })}
              {isTyping && <div className="flex justify-start"><div className="bg-slate-800 px-3 py-2 rounded-xl text-[11px] text-slate-400">Analyzing... 🔍</div></div>}
              <div ref={chatEndRef} />
            </div>
            {chatBlocked ? (
              <div className="p-3 border-t border-purple-500/20 text-center">
                <div className="text-[10px] text-red-400 font-bold">{t.limitReached} {chatTimer}</div>
                <div className="text-[9px] text-slate-500 mt-0.5">{t.orderAudit}</div>
              </div>
            ) : (
              <div className="p-3 border-t border-purple-500/20 flex gap-2">
                <input value={userMsg} onChange={function(e) { setUserMsg(e.target.value); }} onKeyDown={function(e) { if (e.key === 'Enter') handleSendChat(); }} placeholder={t.pasteCa} className="flex-1 bg-slate-900 border border-purple-500/20 rounded-lg px-3 py-1.5 text-[11px] text-white placeholder-slate-500 focus:border-purple-500 focus:outline-none" />
                <button onClick={handleSendChat} disabled={isTyping || !userMsg.trim()} className="bg-purple-500 hover:bg-purple-400 disabled:opacity-40 rounded-lg p-1.5 transition"><Send className="w-3.5 h-3.5 text-white" /></button>
              </div>
            )}
          </div>
        ) : (
          <button onClick={function() { setIsChatOpen(true); }} className="w-14 h-14 bg-gradient-to-r from-purple-500 to-emerald-400 rounded-full shadow-[0_0_20px_rgba(168,85,247,0.5)] flex items-center justify-center hover:scale-110 transition-transform">
