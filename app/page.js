...
// Version: 1.10
// Fixes:
//   1. Use signTransaction + sendRawTransaction (Phantom compat)
//   2. Auto-create recipient ATA if it doesn't exist
//   3. Guard mrdtPrice: wait until > 0 before computing amounts
//   4. Better error surfacing via toast (not just console)
// ============================================================
...
function redirectToWallet(walletType, planVal, currency, form) {
  ...
  if (walletType === 'solflare') {
    ...
    setTimeout(() => { window.location.href = 'https://solflare.com/ul/v1/browse/' + encoded + '?ref=' + ref; }, 500);
  } else {
    window.location.href = 'phantom://v1/browse/' + encoded;
    setTimeout(() => { window.location.href = 'https://phantom.app/ul/browse/' + encoded + '?ref=' + ref; }, 500);
  }
}
...
// ── MRDT amount helper ────────────────────────────────────────
// Returns raw token units (already multiplied by decimals)
function calcMrdtRaw(usdAmount, mrdtPrice) {
  if (!mrdtPrice || mrdtPrice <= 0) throw new Error('Цена $MRDT ещё не загружена, попробуй через секунду');
  return BigInt(Math.round((usdAmount / mrdtPrice) * Math.pow(10, MRDT_DECIMALS)));
}
...
export default function TntHouse() {
  ...
  const [mrdtPrice, setMrdtPrice]         = useState(0);        // start at 0 — guard in calcMrdtRaw
  ...
  useEffect(() => {
    loadTokensFromSupabase().then((data) => { if (data.length > 0) setListedTokens(data); });
  }, []);

  // ── Restore form from URL params (after wallet redirect) ────
  useEffect(() => {
    ...
    if (pName && pCA && pPlan && pCurrency) {
      ...
      // Auto-trigger payment inside wallet browser — wait for price to load first
      if (isInWalletBrowser()) {
        const waitAndPay = async () => {
          // Wait up to 8 s for mrdtPrice > 0
          for (let i = 0; i < 16; i++) {
            const price = parseFloat(localStorage.getItem('tnt_mrdt_price') || '0');
            if (price > 0) {
              triggerPayment(pPlan, pCurrency, restoredForm, pWallet || 'phantom', price);
              return;
            }
            await new Promise((r) => setTimeout(r, 500));
          }
          // Fallback with default price
          triggerPayment(pPlan, pCurrency, restoredForm, pWallet || 'phantom', 0.000013);
        };
        setTimeout(waitAndPay, 1500);
      }
    }
    ...
    const fetchPrice = async () => {
      try {
        ...
        if (data.pairs?.length) {
          ...
          if (p > 0) {
            setMrdtPrice(p);
            localStorage.setItem('tnt_mrdt_price', p.toString()); // cache for wallet redirect
          }
        }
      } catch (e) {}
  ...
  useEffect(() => {
    ...
    const i = setInterval(() => {
      ...
      setLogs((prev) => prev.slice(-12).concat(['[' + new Date().toLocaleTimeString() + '] ' + t]));
    }, 4200);
    return () => clearInterval(i);
  }, []);
  ...
  // Key fix: use signTransaction + sendRawTransaction + create toAta if missing
  const triggerPayment = async (planVal, currency, form, walletType, priceFallback) => {
    ...
    for (let attempt = 0; attempt < 12; attempt++) {
      if (walletType === 'solflare' && window.solflare?.isSolflare) { solanaWin = window.solflare; break; }
      if (window.solana?.isPhantom) { solanaWin = window.solana; break; }
    ...
    try {
      ...
      const receiver   = new PublicKey(WALLET_ADDRESS);
      let   signature  = '';

      // Use cached or freshly fetched price
      const effectivePrice = priceFallback && priceFallback > 0
        ? priceFallback
        : mrdtPrice > 0
          ? mrdtPrice
          : parseFloat(localStorage.getItem('tnt_mrdt_price') || '0.000013');
      ...
      if (currency === 'mrdt') {
        ...
        const rawAmount = calcMrdtRaw(plan.price, effectivePrice);
        const tx        = new Transaction();
        tx.feePayer     = sender;
        tx.recentBlockhash = (await connection.getLatestBlockhash('finalized')).blockhash;

        // FIX: create recipient ATA if it doesn't exist yet
        let toAtaExists = true;
        try { await getAccount(connection, toAta); }
        catch (e) { toAtaExists = false; }

        if (!toAtaExists) {
          tx.add(createAssociatedTokenAccountInstruction(sender, toAta, receiver, mint));
        }

        tx.add(createTransferInstruction(fromAta, toAta, sender, rawAmount));

        setPaymentStatus('Подтверди в кошельке...');

        // FIX: use signTransaction + sendRawTransaction (works on all Phantom versions)
        const signedTx = await solanaWin.signTransaction(tx);
        setPaymentStatus('Отправляем транзакцию...');
        signature = await connection.sendRawTransaction(signedTx.serialize(), { skipPreflight: false });
        ...
        const amountLamports    = Math.floor((plan.price / effectiveSolPrice) * LAMPORTS_PER_SOL);
        const projectAta        = await getAssociatedTokenAddress(new PublicKey(MRDT_CA), receiver);
      ...
      const msg = err?.message || 'Ошибка оплаты';
      showToast('❌ ' + msg, 'error');
    } finally {
  ...
  const handleBack = () => { if (step === 2) setStep(1); else if (step === 3) setStep(2); };
  ...
  const handleBannerWalletSelect = async (walletType) => {
    ...
    try {
      const effectivePrice = mrdtPrice > 0 ? mrdtPrice : parseFloat(localStorage.getItem('tnt_mrdt_price') || '0.000013');
      const resp       = await solanaWin.connect();
      const sender     = new PublicKey(resp.publicKey.toString());
      const connection = new Connection(RPC_URL, 'confirmed');
      const mint       = new PublicKey(MRDT_CA);
      ...
      const rawAmt = calcMrdtRaw(usd, effectivePrice);

      const tx = new Transaction();
      tx.feePayer        = sender;
      tx.recentBlockhash = (await connection.getLatestBlockhash('finalized')).blockhash;

      // Create toAta if needed
      let toAtaExists = true;
      ...
      tx.add(createTransferInstruction(fromAta, toAta, sender, rawAmt));
  ...
  const scrollToForm     = () => { const el = document.getElementById('orderFormsSection'); if (el) el.scrollIntoView({ behavior: 'smooth' }); };
  ...
  const getAmountForTier   = (tier)  => { const usd = tier === 'fast' ? 40 : tier === 'vip' ? 120 : 10;  return mrdtPrice > 0 ? Math.round(usd / mrdtPrice) : '...'; };
  const getAmountForBanner = (days)  => { const usd = days === '2' ? 35 : days === '6' ? 100 : 20;        return mrdtPrice > 0 ? Math.round(usd / mrdtPrice) : '...'; };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white font-mono relative overflow-hidden pb-12">
          ...
          <div className="max-w-7xl mx-auto px-6 py-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-[10px] text-purple-400 block font-bold tracking-widest">TOP NEW TOKENS v1.10</span>
              </div>
            </div>
        ...
        <footer className="border-t border-purple-500/20 mt-12 py-8 bg-slate-950/60 backdrop-blur-lg">
          <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="text-center space-y-1">
              <div className="text-purple-400 font-bold text-sm tracking-widest">TNT HOUSE v1.10</div>
      ...
      {/* ── MODALS ── */}
      {showPayWalletModal && <WalletModal title="Выбери кошелёк для оплаты" onSelect={handleWalletChoice} onClose={() => setShowPayWalletModal(false)} />}
      {showBannerWalletModal && <WalletModal title="Оплата VIP-баннера" onSelect={handleBannerWalletSelect} onClose={() => setShowBannerWalletModal(false)} />}
