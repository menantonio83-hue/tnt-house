...
export default function TntHouse() {
  ...
  // ===== ОПЛАТА + АУДИТ + ДОБАВЛЕНИЕ В ТАБЛИЦУ =====
  const handlePayment = async () => {
    ...
    if (!plan) return;
    if (!window.solana || !window.solana.isPhantom) { showToast('Установи Phantom Wallet', 'error'); return; }
    ...
    try {
      ...
      try {
        ...
        if (auditData.code === 1 && auditData.result?.[formData.contractAddress]) {
          const a = auditData.result[formData.contractAddress];
          auditResult = {
        ...
        const m = new PublicKey(formData.contractAddress);
        const mi = await getMint(c2, m);
        auditResult = {
  ...
  const handleAuditWalletSelect = async (walletType) => {
    ...
    try {
      ...
      const newToken = { name: current.projectName.toUpperCase(), symbol: current.projectName.slice(0,4).toUpperCase()||'NEW', ca: current.ca, price: (Math.random()*0.00005+0.000001).toFixed(8), liquidity: Math.floor(Math.random()*100000+10000), volume24h: Math.floor(Math.random()*90000+20000), priceChange24h: parseFloat((Math.random()*40-10).toFixed(1)), verified: true, dexUrl: `https://dexscreener.com/solana/${current.ca}`, chain: 'solana' };
      setTokens(prev => [newToken, ...prev]);
      setSubmitted(true);
      setFormData({ projectName:'', ca:'', telegram:'' });
      setSelectedToken({ symbol: current.projectName.slice(0,4).toUpperCase()||'NEW', name: current.projectName.toUpperCase(), ca: current.ca, price: newToken.price, liquidity: newToken.liquidity, volume24h:0, priceChange24h:0, verified:true, dexUrl: `https://dexscreener.com/solana/${current.ca}`, chain:'solana' });
                ...
                <div className="bg-gray-800/50 p-6 rounded-2xl border border-gray-700/50 max-w-md mx-auto">
                  ...
                  {step === 3 && (
                    <>
                      ...
                      <div className="flex gap-3 mt-6">
                        <button onClick={handleBack} className="flex-1 bg-gray-700 text-white py-3 rounded-xl font-semibold hover:bg-gray-600 transition">← Назад</button>
                        <button onClick={handlePayment} disabled={!selectedCurrency} className="flex-1 bg-gradient-to-r from-indigo-500 to-purple-600 text-white py-3 rounded-xl font-semibold hover:opacity-90 transition disabled:opacity-40">Запустить ИИ-инспекцию</button>
                      </div>
                    </>
                  )}
                </div>
      ...
      {isBlueprintOpen && (selectedToken || auditResult) && (
        <div className="fixed inset-0 z-[9999] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" onClick={closeBlueprint}>
          <div className="bg-slate-950 border-2 border-purple-500/40 rounded-2xl w-full max-w-lg p-6 shadow-lg" onClick={e => e.stopPropagation()}>
            ...
            {auditResult ? (
              <div className="mt-4 text-sm"><p>Mint: {auditResult.mintAuthority}</p><p>Freeze: {auditResult.freezeAuthority}</p></div>
